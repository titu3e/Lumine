...
import Lumine.modules.sql.notes_sql as sql
from Lumine import dispatcher, LOGGER, OWNER_ID, JOIN_LOGGER, SUPPORT_CHAT
from Lumine.__main__ import DATA_IMPORT
...
# from Lumine.modules.rules import get_rules
import Lumine.modules.sql.rules_sql as rulessql

# from Lumine.modules.sql import warns_sql as warnssql
import Lumine.modules.sql.blacklist_sql as blacklistsql
from Lumine.modules.sql import disable_sql as disabledsql

# from Lumine.modules.sql import cust_filters_sql as filtersql
# import Lumine.modules.sql.welcome_sql as welcsql
import Lumine.modules.sql.locks_sql as locksql
from Lumine.modules.connection import connected
...
def export_data(update: Update, context: CallbackContext):
    ...
    with open("Lumine{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("Lumine{}.backup".format(chat_id), "rb"),
        caption="*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `Lumine-Backup` was specially made for notes.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("Lumine{}.backup".format(chat_id))  # Cleaning file
